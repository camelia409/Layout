...
def main():
    ...
    # Normalise room type name: legacy CSVs may contain 'puja_room'; engine uses 'pooja'
    if "adjacency_rules" in dfs:
        for col in dfs["adjacency_rules"].select_dtypes(include="object").columns:
            dfs["adjacency_rules"][col] = (
                dfs["adjacency_rules"][col].str.replace("puja_room", "pooja", regex=False)
            )

    # Skip temp/working copies that were created as workarounds for file locks
    for skip in ("adjacency_rules_new", "adjacency_rules_fixed"):
        dfs.pop(skip, None)
    ...
    try:
        ...
        safe_index(conn, "idx_ftr_from",  "floor_transformation_rules", "from_room_type")
        safe_index(conn, "idx_ftr_prio",  "floor_transformation_rules", "priority")
        safe_index(conn, "idx_vsr_room",  "vertical_stacking_rules",    "room_type")
        safe_index(conn, "idx_vsr_stack", "vertical_stacking_rules",    "must_stack")
        safe_index(conn, "idx_sr_type",  "strategy_rules",             "rule_type")
        safe_index(conn, "idx_sr_key",   "strategy_rules",             "rule_key")
        conn.commit()
